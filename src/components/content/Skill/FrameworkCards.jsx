...
const FrameworkGrid = () => {
  const renderedFrameworks = useMemo(
    () => frameworks.map(({ Icon, title, color, bg, border, description }) => (
          <div key={title} className={`group glass-effect relative rounded-2xl p-6 flex flex-col items-start hover:bg-zinc-500/10 border-b-0 border-l-2 hover:border-l-[3px] ${border} transition-all duration-100 ease-in`}>
